 ...
 */
public class AlwaysRemapWindowPool implements WindowPool
{
     ...
     */
    public AlwaysRemapWindowPool( String storeName, int blockSize,
    ...
    private synchronized void attemptToMap( int brickIndex )
    {
        brickMiss = 0;
        ...
        BrickElement requestedBrick = brickArray[brickIndex];
        if ( memUsed + brickSize <= availableMem && requestedBrick.getWindow() == null )
        {
            allocateNewWindow( requestedBrick );
            return;
        }

        BrickElement mappedBrick = mappedBricks.get( 0 );
        ...
        if (window.writeOutAndCloseIfFree( readOnly ) )
        {
            mappedBrick.setWindow( null );
            memUsed -= brickSize;
            if ( allocateNewWindow( requestedBrick ) )
                switches++;
        }
    }
     ...
     *
     * The two {@link java.util.List lists} coming back are also sorted where the first element
     ...
     * {@link org.neo4j.kernel.impl.nioneo.store.PersistenceRow persistence rows}.
     *
